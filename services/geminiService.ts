...
export const chatWithExpert = async (
  ...
  const systemInstruction = `
    ...
    ВНИМАНИЕ: Вы находитесь в режиме обсуждения сохраненного проекта. Вы должны вести разговор СУГУБО в разрезе этого проекта. Если пользователь задает вопросы на отвлеченные темы, не связанные с реставрацией конкретно этого объекта, вежливо откажитесь и попросите задавать вопросы только по текущему проекту.` : 'Контекстного проекта сейчас нет (главная страница). Вы можете общаться на любые темы, связанные с искусством, консервацией и реставрацией.'}
